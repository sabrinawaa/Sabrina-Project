...
#%%
plt.scatter(xns,pxns)
#%%
x0i=[]
px0i=[]
tunei=[]
for a in range(len(xns)):
     if abs(tunes[a]-0.75)<0.0001:
         x0i.append(xns[a])
         px0i.append(pxns[a])
         tunei.append(tunes[a])
fig2,ax2=plt.subplots()
for i in range (1,7774,100):
    # if i <10:
    #     name=folder+"track.obs0001.p000"+str(i)
    # elif 9<i<100:   
    #     name=folder+"track.obs0001.p00"+str(i)
...
im3=ax3.scatter(x0is,px0is,c=tuneis,s=5,cmap=plt.cm.jet)
plt.xlabel("X0")
plt.ylabel("Px0")
...
#%%
area=[]
tol=np.linspace(0.000001,0.0001,100)
for t in tol:
    x0i=[]
    px0i=[]
    tunei=[]
...
plt.figure()
plt.scatter(turns[1:], areas[1:], s=1, marker='x')    
plt.xlabel("no_turns")
plt.ylabel("Area")