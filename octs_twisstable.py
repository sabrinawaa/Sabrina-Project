...
for k in oct_names:
    for j in strengths:
        twiss_name="twiss.oct="+str(k)+"k3=" +str(j)+".tfs"
        twissum_name="twissum.oct="+str(k)+"k3=" +str(j)+".tfs"
        ...
        data=twiss_sum.loc[:,twiss_sum.columns.isin([ 'ALPHA_C', 'ALPHA_C_P','ALPHA_C_P2','ALPHA_C_P3','DQ1','DQ2'])].astype(np.float64)
